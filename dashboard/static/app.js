...
function getOpencodePluginMode(snapshot) {
  ...
  const hasOhMy = entries.some((entry) => entry.startsWith("oh-my-opencode"));
  const hasSuperpowers = entries.some((entry) => entry.includes("superpowers"));

  if (hasOhMy && !hasSuperpowers) return "oh-my-opencode";
  if (!hasOhMy && hasSuperpowers) return "custom-mixed";
  if (hasOhMy && hasSuperpowers) return "custom-mixed";
  return "none";
}
...
function formatPluginMode(mode) {
  switch (mode) {
    case "oh-my-opencode":
      return "Oh My OpenAgent only";
    case "custom-mixed":
      return "custom / mixed plugin profile";
    default:
...
function syncActionButtons() {
  const codexMode = state.snapshot?.codexOrchestratorMode || "hybrid-tools-only";
  ...
  if (dom.visibilitySummary) {
    dom.visibilitySummary.textContent = visibilityMode === "on"
      ? "Shared skills: visible to Codex/OpenCode when supported"
      : "Shared skills: hidden";
  }
}
...
function renderToolDetail(tool) {
  ...
  if (!tool) {
    dom.profileSpotlight.innerHTML = "<p class=\"muted\">Select a profile to view details.</p>";
    return;
  }

  const fragment = dom.toolCardTemplate.content.cloneNode(true);
  fragment.querySelector(".tool-kicker").textContent = "Profile Details";
  ...
  fragment.querySelectorAll(".tool-section").forEach((section) => {
    ...
    if (heading === "Editable Files" || heading === "Status Details" || heading === "Restart / Reload Notes") {
      section.classList.add("tool-section-wide");
    }

    if (heading === "Constraints" || heading === "Restart / Reload Notes" || heading === "Editable Files") {
...
function renderFileList(files) {
  ...
  if (groups.size === 0) {
    ...
    empty.textContent = isFiltered ? "No files match your search." : "No discovered files yet.";
...
function renderBench(bench) {
  ...
  bench.forEach((entry) => {
    ...
    fragment.querySelector(".copy-prompt").addEventListener("click", async () => {
      ...
      setActionStatus(`Copied prompt: ${entry.title}`, "success");
    });
...
async function loadFile(file, { activateSection = true } = {}) {
  if (isActiveFileDirty() && file.path !== state.activePath) {
    setActionStatus("Save or discard changes before opening another file.", "error");
    return;
  }
  ...
  dom.editorPath.textContent = `${formatDisplayPath(file.path)} | ${payload.editable ? "Editable source file" : "Read-only generated view"}`;
...
async function runAction(endpoint, body = {}) {
  ...
  setActionStatus("Running operation…", "pending");
  setOperationStatus("Running operation…", "pending");

  const payload = await request(endpoint, {
    method: "POST",
    body: JSON.stringify(body)
  });
  setOperationStatus(payload.output || "Operation completed.", "success");
  setActionStatus(payload.output || "Operation completed.", "success");
  await refreshState();
}
...
async function refreshState({ announce = false } = {}) {
  ...
  dom.generatedAt.textContent = `Last refreshed: ${snapshot.generatedAt} | Shared skills: ${snapshot.globalSuperpowersVisibility}`;
  ...
  if (state.activePath && snapshot.managedFiles.some((file) => file.path === state.activePath)) {
    ...
    if (isActiveFileDirty()) {
      ...
      dom.editorPath.textContent = `${formatDisplayPath(active.path)} | ${active.editable ? "Editable source file" : "Read-only generated view"}`;
  ...
  if (announce) {
    setActionStatus("State refreshed.", "success");
  }
}
...
dom.refreshState.addEventListener("click", () => {
  setActionStatus("Refreshing state…", "pending");
...
setActiveSection(state.activeSection);
resetEditor();
setActionStatus("Ready.", "idle");
setOperationStatus("Select an operation to run.", "idle");
