...
`bootstrap-home.sh` always handles local generated-file wiring first. `ais` registration is best-effort when available. External plugin/extension installs (Claude/Gemini/OpenCode add-ons) are not auto-managed by bootstrap.
