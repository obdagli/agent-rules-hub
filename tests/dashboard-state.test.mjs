...
test("evaluateProfileStatus keeps OpenCode managed when instruction delivery is wired", () => {
  ...
  assert.match(status.summary, /instruction delivery|plugin profile/i);
});

test("evaluateProfileStatus marks untracked Codex orchestrator modes as detected even when delivery is wired", () => {
  const status = evaluateProfileStatus("codex", {
    syncOk: true,
    codexOrchestratorMode: "custom",
  ...
  assert.match(status.summary, /tracked presets|orchestrator mode/i);
});
...
test("evaluateProfileStatus marks Claude with plugin and synced policy as managed", () => {
  ...
  assert.equal(status.status, "managed");
  assert.match(status.summary, /managed|external|state/i);
});

test("evaluateProfileStatus keeps Codex managed even when shared skill discovery is unmanaged", () => {
  const status = evaluateProfileStatus("codex", {
    syncOk: true,
    codexOrchestratorMode: "hybrid-tools-only",
    globalSuperpowersVisible: true,
    codexSuperpowersManaged: false,
    opencodeInstructionsConfigured: true,
    claudePlugins: [],
  ...
  assert.equal(status.status, "managed");
  assert.match(status.summary, /managed|hybrid/i);
});

test("evaluateProfileStatus keeps Gemini managed and reports unmanaged linked source as runtime detail", () => {
  ...
  assert.equal(status.status, "managed");
  assert.match(status.summary, /managed|runtime|state/i);
  assert.match(status.details.join(" "), /not linked|local checkout|external/i);
});
...
test("evaluateProfileStatus keeps Claude managed when plugin is disabled and reports it as runtime state", () => {
  const status = evaluateProfileStatus("claude", {
    syncOk: true,
    globalSuperpowersVisible: true,
    codexSuperpowersManaged: true,
  ...
  assert.equal(status.status, "managed");
  assert.match(status.summary, /managed|external|state/i);
  assert.match(status.details.join(" "), /disabled/i);
});

test("evaluateProfileStatus keeps Gemini managed when extension is disabled and reports it as runtime state", () => {
  const status = evaluateProfileStatus("gemini", {
    syncOk: true,
    globalSuperpowersVisible: true,
    codexSuperpowersManaged: true,
...
test("buildDashboardState keeps Gemini source notes aligned with the active extension state", async () => {
  const { tempHome, binDir } = await makeFakeDashboardHome();
  const probeScript = `
    import { buildDashboardState } from ${JSON.stringify(pathToFileURL(path.join(REPO_ROOT, "lib", "state.mjs")).href)};
    const state = await buildDashboardState();
    const gemini = state.tools.find((tool) => tool.id === "gemini");
    const source = gemini.activeSources.find((entry) => entry.label === "Gemini extensions detected");
  ...
  try {
    ...
    assert.match(payload.note, /locally linked superpowers extension|owned by Gemini/i);
  } finally {
    await fs.rm(tempHome, { recursive: true, force: true });
  }
});

test("buildDashboardState marks Codex shared skill discovery as hub-managed when the live symlink points at the managed checkout", async () => {
  const { tempHome, binDir, superpowersRoot } = await makeFakeDashboardHome();
  const probeScript = `
    import { buildDashboardState } from ${JSON.stringify(pathToFileURL(path.join(REPO_ROOT, "lib", "state.mjs")).href)};
    const state = await buildDashboardState();
    const codex = state.tools.find((tool) => tool.id === "codex");
    const source = codex.activeSources.find((entry) => entry.label === "Shared skill discovery");
    console.log(JSON.stringify({
  ...
  try {
    ...
    assert.match(payload.note, /resolves to the local superpowers checkout|shared skill path/i);
  } finally {
    await fs.rm(tempHome, { recursive: true, force: true });
  }
});
